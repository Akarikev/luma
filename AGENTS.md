...
- On Debian/Ubuntu, package architecture **`amd64` is 64-bit x86**, the same ISA toolchains label **`x86_64`**; Linux release artifacts named either way refer to the same build unless a separate ARM (`aarch64`) job exists.
...
- On WebKitGTK (Linux Tauri), large `box-shadow` on rounded shells often paints as a square; prefer `filter: drop-shadow(...)` for the main chrome. Keep a single rounded clip on the app root and place full-screen overlays (e.g. settings) in the same stacking context (`absolute` inside the clipped wrapper) so corners stay consistent.
